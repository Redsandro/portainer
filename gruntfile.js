...
module.exports = function(grunt) {
    ...
    grunt.registerTask('checkLocalDocker', [
       'shell:extractDockerBinary:linux:' + hostarch,
       'shell:Docker2Path'
    ]);
    ...
    grunt.initConfig({
            ...
            assets: {
                files: [{
                    ...
                    },
                    {
                        ...
                        src: '**',
                        expand: true,
                        cwd: '<%= workdir %>assets/images/'
                    },
                    {
                        dest: '<%= distdir %>ico',
                        src: '**',
                        expand: true,
                        cwd: '<%= workdir %>assets/ico'
                    }
                ]
            }
        },
        eslint: {
            ...
            options: {
                configFile: '<%= workdir %>.eslintrc.yml'
            }
        },
        html2js: {
            app: {
        ...
        shell: {
            ...
            syncDist: {
                command: 'rsync -avr --delete-before dist /src-volume'
            },
            rm: {
                command: 'if [ -z "$(docker container inspect portainer-dev 2>&1 | grep "Error:")" ]; then docker container rm -f portainer-dev; fi'
            },
            run: {
                command: 'docker run -d -p 9001:9000 -v portainer-src-volume:/src -v /tmp/portainer:/data -v /var/run/docker.sock:/var/run/docker.sock:z --name portainer-dev -w /src/dist portainer/base ./portainer-linux-' + hostarch + ' --no-analytics -a /src/dist'
            },
